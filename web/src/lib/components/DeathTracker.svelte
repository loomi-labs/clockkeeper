<script lang="ts">
	import type { Game, Character, Death } from '~/lib/gen/clockkeeper/v1/clockkeeper_pb';
	...
	const teamOrder = [Team.TOWNSFOLK, Team.OUTSIDER, Team.MINION, Team.DEMON, Team.TRAVELLER] as const;
...
<div class="rounded-lg border border-border bg-surface p-4">
	<div class="flex items-center justify-between mb-3">
		<h3 class="text-lg font-semibold text-primary">
			...
			{#if displayDeaths.length > 0}
				<span class="ml-1 text-sm font-normal text-secondary">({displayDeaths.length})</span>
			{/if}
		</h3>
		{#if !readonly}
			<button
			...
			<div class="space-y-3">
				{#each teamOrder as team}
					...
					{#if chars && chars.length > 0}
						<div>
							<span class="mb-1 block text-xs font-semibold uppercase tracking-wide {teamNameColors[team] ?? 'text-secondary'}">
								{teamLabels[team] ?? ''}
							</span>
							<div class="grid gap-1 sm:grid-cols-2">
								{#each chars as char (char.id)}
									...
									>
										...
										/>
										<span class="text-sm font-medium text-primary">{char.name}</span>
									</button>
								{/each}
							</div>
						</div>
	...
	<!-- Deaths list -->
	{#if displayDeaths.length === 0}
		<p class="py-4 text-center text-sm text-muted">{showPhaseSummary ? 'No deaths this phase.' : 'No deaths yet.'}</p>
	{:else}
		<div class="space-y-1">
			{#each displayDeaths as death (death.id)}
				...
				<div class="flex items-center gap-3 rounded-lg bg-element/50 px-3 py-2">
					{#if char}
						...
						/>
					{:else}
						<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-element text-sm text-secondary">?</div>
					{/if}
					<div class="min-w-0 flex-1">
						<span class="text-sm font-medium text-primary">{char?.name ?? death.roleId}</span>
						<span class="ml-2 text-xs text-muted">{phaseLabel(death.phaseId)}</span>
					</div>
					<!-- Ghost vote indicator -->
					<button
						onclick={() => {
							if (!readonly && death.ghostVote) return;
							if (!readonly) onuseghostvote(death.id);
						}}
					...
					>
						<!-- Skull icon -->
						<svg class="h-5 w-5" viewBox="0 0 24 24" fill={death.ghostVote ? 'none' : 'currentColor'} stroke="currentColor" stroke-width={death.ghostVote ? '1.5' : '0'}>
							{#if death.ghostVote}
								<!-- Empty skull (vote used) with strikethrough -->
								<path stroke-linecap="round" stroke-linejoin="round" d="M12 2C7.58 2 4 5.58 4 10c0 2.76 1.34 5.2 3.4 6.72V20a1 1 0 001 1h7.2a1 1 0 001-1v-3.28C18.66 15.2 20 12.76 20 10c0-4.42-3.58-8-8-8zm-2 17v-1h4v1h-4zm0-3h1v2h2v-2h1v2h-4zm5.6-2.08l-.6.46V17h-6v-2.62l-.6-.46A5.94 5.94 0 016 10c0-3.31 2.69-6 6-6s6 2.69 6 6a5.94 5.94 0 01-2.4 3.92z" />
								<line x1="4" y1="4" x2="20" y2="20" stroke-width="2" />
							{:else}
								<!-- Filled skull (vote available) -->
								<path d="M12 2C7.58 2 4 5.58 4 10c0 2.76 1.34 5.2 3.4 6.72V20a1 1 0 001 1h7.2a1 1 0 001-1v-3.28C18.66 15.2 20 12.76 20 10c0-4.42-3.58-8-8-8zm-1 15v-2h2v2h-2zm4-7a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zm-5 0a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z" />
							{/if}
						</svg>
					</button>
					{#if !readonly}
	...
	{#if showPhaseSummary && allDeaths.length > 0}
		<p class="mt-3 text-center text-xs text-muted">
			{allDeaths.length} total {allDeaths.length === 1 ? 'death' : 'deaths'} across all phases
		</p>
	{/if}
</div>
