      ...
      <section>
        ...
        <div class="space-y-2">
          {#each chars as char (char.id)}
            <a
              href="/almanac/{char.id}"
              class="card-slate block rounded-lg border p-4 transition-shadow hover:shadow-md {teamCardColors[
                char.team
              ...
              <div class="flex gap-4">
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p class="mt-1 text-sm text-secondary">
                    {@html formatReminder(char.ability)}
                  </p>
                  ...
                  {#if char.firstNightReminder || char.otherNightReminder}
                    <div class="mt-2 space-y-1 border-t border-border/50 pt-2">
                      ...
                      {#if char.otherNightReminder}
                        <div class="flex items-start gap-1.5 text-xs">
                          <svg
                            class="mt-0.5 h-3 w-3 shrink-0 text-amber-400"
                            fill="currentColor"
                            viewBox="0 0 20 20"
                            ><path
                              fill-rule="evenodd"
                  ...
                  {#if char.reminders.length > 0 || char.remindersGlobal.length > 0}
                    <div class="mt-2 flex flex-wrap gap-1">
                      ...
                      {#each char.remindersGlobal as reminder}
                        <span
                          class="rounded bg-element px-1.5 py-0.5 text-xs italic text-muted"
                          >{reminder} (global)</span
                        >
                      {/each}
                    </div>
                  {/if}
