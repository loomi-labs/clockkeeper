<script lang="ts">
  ...
  async function createCustom() {
    try {
      const resp = await client.createScript({
        name: "New Script",
        characterIds: [],
      });
      if (resp.script) {
...
<div class="space-y-8">
  ...
  {:else}
    ...
    {#if systemScripts.length > 0}
      <section>
        <h2 class="mb-3 font-[Goudy_Stout] text-base text-primary">
          Base Editions
        </h2>
          ...
          {#each systemScripts as sysScript (sysScript.id)}
            ...
            >
              ...
              <p class="mt-3 text-sm text-secondary">
                {sysScript.characterIds.length} characters
              </p>
            </a>
          {/each}
        </div>
    ...
    <section>
      ...
      {#if userScripts.length === 0}
        <p class="text-muted">
          No saved scripts yet. Create one from an edition or start from
          scratch.
        </p>
      {:else}
        <div class="space-y-2">
