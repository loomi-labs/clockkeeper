    ...
    <div class="flex flex-wrap gap-1.5">
      {#each editions as ed}
        <button
          ...
          class="rounded-full px-3 py-1 text-xs font-medium transition-colors {editionFilter ===
          ed.id
            ? ed.active
            : 'bg-element text-secondary hover:text-primary'}"
        >
          {ed.label}
        </button>
      {/each}
    ...
    <div class="flex flex-wrap gap-1.5">
      {#each teams as t}
        <button
          ...
          class="rounded-full px-3 py-1 text-xs font-medium transition-colors {teamFilter ===
          t.id
            ? t.active
            : 'bg-element text-secondary hover:text-primary'}"
        >
